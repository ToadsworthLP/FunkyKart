...
public class KartEntity extends Entity {
    ...
    private KartEntity.Location location;
    private KartEntity.Location lastLocation;
    ...
    public KartEntity(EntityType<? extends KartEntity> entityType, World world) {
    ...
    protected void initDataTracker() {
        ...
        this.dataTracker.startTracking(BOAT_TYPE, KartEntity.Type.OAK.ordinal());
    ...
    public void pushAwayFrom(Entity entity) {
        if (entity instanceof KartEntity) {
    ...
    public void tick() {
        ...
        if (this.location != KartEntity.Location.UNDER_WATER && this.location != KartEntity.Location.UNDER_FLOWING_WATER) {
    ...
    private KartEntity.Location checkLocation() {
        KartEntity.Location location = this.getUnderWaterLocation();
        if (location != null) {
            this.waterLevel = this.getBoundingBox().maxY;
            return location;
        } else if (this.checkBoatInWater()) {
            return KartEntity.Location.IN_WATER;
        } else {
            float f = this.method_7548();
            if (f > 0.0F) {
                this.field_7714 = f;
                return KartEntity.Location.ON_LAND;
            } else {
                return KartEntity.Location.IN_AIR;
            }
        }
    }
    ...
    @Nullable
    private KartEntity.Location getUnderWaterLocation() {
        Box box = this.getBoundingBox();
                ...
                for(int q = m; q < n; ++q) {
                    ...
                    if (fluidState.isIn(FluidTags.WATER) && d < (double)((float)mutable.getY() + fluidState.getHeight(this.world, mutable))) {
                        if (!fluidState.isStill()) {
                            return KartEntity.Location.UNDER_FLOWING_WATER;
                        }
        ...
        return bl ? KartEntity.Location.UNDER_WATER : null;
    }
    ...
    private void updateVelocity() {
        ...
        if (this.lastLocation == KartEntity.Location.IN_AIR && this.location != KartEntity.Location.IN_AIR && this.location != KartEntity.Location.ON_LAND) {
            ...
            this.location = KartEntity.Location.IN_WATER;
        } else {
            if (this.location == KartEntity.Location.IN_WATER) {
                f = (this.waterLevel - this.getY()) / (double)this.getHeight();
                this.velocityDecay = 0.9F;
            } else if (this.location == KartEntity.Location.UNDER_FLOWING_WATER) {
                e = -7.0E-4D;
                this.velocityDecay = 0.9F;
            } else if (this.location == KartEntity.Location.UNDER_WATER) {
                f = 0.009999999776482582D;
                this.velocityDecay = 0.45F;
            } else if (this.location == KartEntity.Location.IN_AIR) {
                this.velocityDecay = 0.9F;
            } else if (this.location == KartEntity.Location.ON_LAND) {
    ...
    protected void readCustomDataFromNbt(NbtCompound nbt) {
        if (nbt.contains("Type", 8)) {
            this.setBoatType(KartEntity.Type.getType(nbt.getString("Type")));
        }
        ...
        if (!this.hasVehicle()) {
            if (onGround) {
                if (this.fallDistance > 3.0F) {
                    if (this.location != KartEntity.Location.ON_LAND) {
    ...
    public void setBoatType(KartEntity.Type type) {
        this.dataTracker.set(BOAT_TYPE, type.ordinal());
    }

    public KartEntity.Type getBoatType() {
        return KartEntity.Type.getType((Integer)this.dataTracker.get(BOAT_TYPE));
    }
    ...
    public boolean isSubmergedInWater() {
        return this.location == KartEntity.Location.UNDER_WATER || this.location == KartEntity.Location.UNDER_FLOWING_WATER;
    }
    ...
    static {
        DAMAGE_WOBBLE_TICKS = DataTracker.registerData(KartEntity.class, TrackedDataHandlerRegistry.INTEGER);
        DAMAGE_WOBBLE_SIDE = DataTracker.registerData(KartEntity.class, TrackedDataHandlerRegistry.INTEGER);
        DAMAGE_WOBBLE_STRENGTH = DataTracker.registerData(KartEntity.class, TrackedDataHandlerRegistry.FLOAT);
        BOAT_TYPE = DataTracker.registerData(KartEntity.class, TrackedDataHandlerRegistry.INTEGER);
        LEFT_PADDLE_MOVING = DataTracker.registerData(KartEntity.class, TrackedDataHandlerRegistry.BOOLEAN);
        RIGHT_PADDLE_MOVING = DataTracker.registerData(KartEntity.class, TrackedDataHandlerRegistry.BOOLEAN);
        BUBBLE_WOBBLE_TICKS = DataTracker.registerData(KartEntity.class, TrackedDataHandlerRegistry.INTEGER);
    }
    ...
    public static enum Type {
        ...
        public static KartEntity.Type getType(int type) {
            KartEntity.Type[] types = values();
        ...
        public static KartEntity.Type getType(String name) {
            KartEntity.Type[] types = values();
